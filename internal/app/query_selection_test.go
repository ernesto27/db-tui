...
func TestQueryMouseSelectionResolvesLogicalSQL(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, ansi.Strip(updated.query.editor.View()), ansi.Strip(updated.query.editorView(nil)))
			assert.NotEqual(t, updated.query.editor.View(), updated.query.editorView(nil))
		})
	}
}
