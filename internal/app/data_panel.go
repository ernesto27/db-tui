...
func (m dataModel) view(status dataStatus, layout appLayout, focused bool) string {
	...
	case status.tableLoadErr != nil:
		return panelStyle(layout.data.width, layout.data.height, focused).Render("Unable to load PostgreSQL tables:\n" + sanitizeText(status.tableLoadErr.Error()))
	case status.noTables:
		return panelStyle(layout.data.width, layout.data.height, focused).Render("No public tables found.")
	case m.loading:
